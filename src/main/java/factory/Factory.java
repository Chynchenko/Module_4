...
public class Factory {
    ...
    private int countDetailAssembly;
    ...
    public void startRobots() throws InterruptedException {
        ...
        fourthRobot.start();
        fourthRobot.join();
        Thread fifthRobot = new Thread(makeDetailAssembly);
        fifthRobot.start();
        fifthRobot.join();
    }
    ...
    private final Runnable makeDetailAssembly = () -> {
        LOGGER.info("Fifth robot ({}) starts to work", Thread.currentThread().getName());
        while (countDetailAssembly <= 100) {
            ...
            if (availableFuel < countFuelNeed) {
                ...
                countDetailAssembly += 10;
                LOGGER.info("Fifth robot ({}) make {} of work",
                        Thread.currentThread().getName(), countDetailAssembly);
                try {
                    TimeUnit.SECONDS.sleep(1);
                } catch (InterruptedException e) {
                    throw new RuntimeException(e);
